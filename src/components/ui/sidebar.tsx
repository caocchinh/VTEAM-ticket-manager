...
function SidebarProvider({
  ...
  // We use openMobileProp and setOpenMobileProp for control from outside the component.
  ...
  const toggleSidebar = useCallback(() => {
    return isMobile
      ? setOpenMobile((__openMobile) => !__openMobile)
...
function Sidebar({
  ...
  if (isMobile) {
    return (
      <Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
        <SheetContent
          className="z-[100006] w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
          data-mobile="true"
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "fixed bottom-0 z-10 hidden h-[100dvh] w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex",
          side === "left"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "-translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-[100] hidden w-4 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=right]:left-0 sm:flex",
