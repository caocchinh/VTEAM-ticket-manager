import { fetchTeacherVerification } from "@/lib/SpreadSheet";
...
export async function GET() {
  try {
    ...
    try {
      const response = await fetchTeacherVerification();
      if (response.error || !response.data) {
        return createApiError(
          "TEACHER_VERIFICATION_FETCH_FAILED",
          HTTP_STATUS.BAD_REQUEST
        );
      }
      return Response.json(response.data as TeacherVerificationInfo[]);
    } catch (error) {
      return createApiError(
        "TEACHER_VERIFICATION_FETCH_FAILED",
        HTTP_STATUS.INTERNAL_SERVER_ERROR,
    ...
    console.error("Error fetching teacher verification:", error);
    return createApiError(
